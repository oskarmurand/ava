...
class xavierReporter {
	...
	test(test) {
		...
		if (test.error) {
			test.error.value = typeof test.error.values[0] !== 'undefined' ? test.error.values[0].value : 'missing expected value';
			output.push(
				indentString('<failure>', 4),
				indentString(`expected: ${stripAnsi(xmlEscape(test.error.value))}`, 6),

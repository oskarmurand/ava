	...
	test(test) {
		...
		if (test.error) {
			output.push(
				...
				indentString(`expected: ${stripAnsi(test.error.values[0].value || 'missing expected value')}`, 6),
				...
				indentString(`error: ${stripAnsi(test.error.stack).replace(/<anonymous>/gi, "&#060;anonymous&#062;")}`, 6),
				'',
				indentString(`full log: ${util.inspect(test, false, null).replace(/<anonymous>/gi, "&#060;anonymous&#062;")}`, 6),
				indentString('</failure>', 4)
			);
		}
		output.push(

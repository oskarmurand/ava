...
class xavierReporter {
	...
	test(test) {
		...
		const title = fullTitle.split(" ").pop();

		output.push(
			`<testsuite tests="1" failures="${failed}"${skipped}${todo} errors="0" name="${title}" time="${duration}" >`,
			`  <testcase name="${title}">`);
		...
		if (test.error) {
			output.push(
				...
				'        at: ' + indentString(getSourceFromStack(test.error.stack, 1)),
				`        file: ${file}`,
				'      ...',
				'    </failure>'
			);
